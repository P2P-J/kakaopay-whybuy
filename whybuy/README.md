...
- **Codex 실로드 검증은 개발 환경에서 불가능하다.** `.codex-plugin/plugin.json`은 규격에 맞춰 작성했으나, 실제 Codex가 이 플러그인을 로드해 스킬을 인식하는지는 아엔 로컬/협력 계정에서만 확인 가능하다. 그 외 모든 것은 위 명령으로 Codex 없이 재현된다.
