...
스킬(SKILL.md) ──▶ tools/run_skill.py (파이프라인 순수 함수 + 얇은 CLI)
 ...
 (공시→유형)     RULE-EARN/RET/DIV/GRW/NEW/THM/PRC   (급변 감지)
   ...
   └──▶ render.py(조립·문구 textstore 단일출처) ──▶ compliance_gate ──▶ reports/
                     ▲                                             
        mcp/dart_client(픽스처/캐시, as_of 필터 일괄) ◀── mcp/server.py(MCP 도구 10종)
...
- **Codex 실로드 — 부분 확인(아엔 로컬, 2026-07-10).** MCP 서버가 Codex CLI(v0.143.0)에 등록·연결되어 도구 10종이 인식됨을 실증 확인. 스킬 3종과 플러그인 카탈로그 등록은 마켓플레이스 스냅샷 규격상 별도 작업이 필요하여 정적 검증(151 테스트 + 컨트랙트 3종)으로 대체.
