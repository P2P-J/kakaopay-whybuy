...
`src/verification/`에는 개발 중 실행한 검증 기록(`final-verification.txt` 최종 검증 요약, `link-check.txt` DART 원문 링크 점검 결과)이 있다. 대화 로그가 아니라 검증 재현 결과이며, 개발 대화 원본은 `logs/claude-code/`에만 있다.
